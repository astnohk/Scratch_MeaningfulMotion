...
#define WEIGHTED_FILTER_SIZE 5
	...
	// Test Output
	pnm.copy(PORTABLE_GRAYMAP_BINARY, size.width, size.height, 255, Pyramid[0], 1.0);
	img_pnm = pnm.Data();
	for (x = 0; x < size.width * size.height; x++) {
		if (img_pnm[x] < 0) {
			img_pnm[x] = 0;
		} else if (img_pnm[x] > 255) {
			img_pnm[x] = 255;
		}
	}
	img_pnm = nullptr;
	...
	for (l = 1; l < Level; l++) {
		...
		for (x = 0; x < size_l.width; x++) {
			for (y = 0; y < size_l.height; y++) {
				...
				for (m = 0; m < WEIGHTED_FILTER_SIZE; m++) {
					ym = 2 * y + m - WEIGHTED_FILTER_SIZE / 2;
					if (ym < 0) {
						ym = abs(ym) - 1;
					...
					for (n = 0; n < WEIGHTED_FILTER_SIZE; n++) {
						xn = 2 * x + n - WEIGHTED_FILTER_SIZE / 2;
						if (xn < 0) {
							xn = abs(xn) - 1;
		...
		pnm.copy(PORTABLE_GRAYMAP_BINARY, size_l.width, size_l.height, 255, Pyramid[l], 1.0);
		img_pnm = pnm.Data();
